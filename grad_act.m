function [r,A] = grad_act(x,index_par, c_const,epsilon);
%
% Purpose:
%
...
%
Acomplete = Acomplete (index_par, :); % active variables
index_aux = (sum (abs(Acomplete),1)~=0); 
Acomplete = Acomplete(:, index_aux);
c_const = c_const(index_aux);


A         = Acomplete(:,find(c_const >= -epsilon)); % gradients of constraints
r         = size(A,2);% active constraints
%
% End of grad_act.