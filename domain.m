function [feasible,c_const] = domain(x, qvec, index_par, filternm);
%
% Purpose:
%
...
filter_func = str2func (filternm);

qvec(index_par) = x;
x = qvec;
p = cell2struct(mat2cell(x, ones(np, 1), [1]),parnm);


c_const = func_const(x);
m       = size(c_const,1);

if sum(c_const <= 0) == m  && filter_func(p)
   feasible = 1;
else
   feasible = 0;
end
