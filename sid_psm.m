function [x_final,f_final,histout] = sid_psm(x_initial, qvec, data, auxData, index_par,const,output, filternm)
%
% Purpose:
%
...
n = size(qvec(index_par),1);

fevals_max = max (2000, 2000*n); %evaluations
...
%
   [feasible,c_const] = domain(x_initial,qvec, index_par, filternm);
   if ~feasible
...
%
   epsilon = epsilon_ini;
   [r,A]   = grad_act(x_initial,index_par, c_const,epsilon);
...
x = x_initial;  
%x         = qvec(index_par);
f         = func_f(x, qvec,index_par, data, auxData);
...
%
              if const
      	         [feasible,c_const_temp] = domain(xtemp,qvec, index_par, filternm);
              end
%
...
%
              if (~const) | feasible 
                 ...
                 if ~match
                    ftemp     = func_f(xtemp,qvec,index_par, data, auxData);
                    func_eval = func_eval + 1;
...
%
   if const
      ...
      [r,A]    = grad_act(x,index_par, c_const,epsilon);
      [D,deg]  = gen(pss,n,const,r,A,tol_degconst);
      nD       = size(D,2); 
      max_D    = max(sqrt(sum(D.^2)));
      if deg
